...
async function handleBonus(request, env) {
  ...
  if (pts === 0) {
    await supabase.from('bonus_botellines').delete().eq('email', email)
  } else {
    const { error } = await supabase.from('bonus_botellines').upsert(
...
async function handleParticipant(request, env) {
  ...
  if (request.method === 'DELETE') {
    const { email } = body
    if (!email) return json({ error: 'Missing email' }, 400)
    await supabase.from('participants_botellines').delete().eq('email', email)
    return json({ ok: true })
  }
  ...
  if (request.method === 'POST') {
    ...
    const { error } = await supabase.from('participants_botellines').upsert(
...
async function handleResults(request, env) {
  ...
  const { error } = await supabase.from('results_botellines').upsert(
...
async function handlePayment(request, env) {
  ...
  if (!email) return json({ error: 'Missing email' }, 400)
  if (!paid) {
    await supabase.from('payments_botellines').delete().eq('email', email)
  } else {
    const { error } = await supabase.from('payments_botellines').upsert(
...
async function handleMeta(request, env) {
  ...
  const { error } = await supabase.from('meta_botellines').upsert(updates, { onConflict: 'id' })
  if (error) return json({ error: error.message }, 500)
  return json({ ok: true })
}
